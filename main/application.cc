...
void Application::CheckNewVersion() {
    // QwenPaw direct mode: skip the official OTA check and activation flow
    if (strlen(CONFIG_QWENPAW_WS_URL) > 0) {
        ESP_LOGI(TAG, "QwenPaw direct mode enabled, skip version check and activation");
        return;
    }
...
void Application::InitializeProtocol() {
    ...
    if (strlen(CONFIG_QWENPAW_WS_URL) > 0) {
        // QwenPaw direct mode: always use the WebSocket protocol
        ESP_LOGI(TAG, "QwenPaw direct mode, using WebSocket protocol");
